...
const ui = {
  ...
  joystickProfileName: null,
  joystickProfileStatus: null,
  joystickControllerType: null,
  joystickControllerPort: null,
  ...
  joystickTargetLibrary: null,
  joystickControllerPreview: null,
  joystickProfileNewButton: null,
  joystickProfileDuplicateButton: null,
  joystickProfileDeleteButton: null,
...
function cacheUi() {
  ...
  ui.joystickProfileName = document.getElementById("joystick-profile-name");
  ui.joystickProfileStatus = document.getElementById("joystick-profile-status");
  ui.joystickControllerType = document.getElementById("joystick-controller-type");
  ui.joystickControllerPort = document.getElementById("joystick-controller-port");
  ...
  ui.joystickTargetLibrary = document.getElementById("joystick-target-library");
  ui.joystickControllerPreview = document.getElementById("joystick-controller-preview");
  ui.joystickProfileNewButton = document.getElementById("joystick-profile-new-button");
  ui.joystickProfileDuplicateButton = document.getElementById("joystick-profile-duplicate-button");
  ui.joystickProfileDeleteButton = document.getElementById("joystick-profile-delete-button");
...
function setupQueueBuilder() {
  ...
  if (ui.joystickProfileSelect) {
    ui.joystickProfileSelect.addEventListener("change", () => {
      const nextProfileId = ui.joystickProfileSelect.value || "";
      setActiveJoystickProfile(nextProfileId);
      renderJoystickEditor();
      renderJoystickLiveInputs();
      void saveJoystickMappings(`Active controller profile switched to ${getActiveJoystickProfile()?.name || "selected profile"}`);
    });
  }
  if (ui.joystickProfileName) {
    ui.joystickProfileName.addEventListener("input", () => {
      const profile = getActiveJoystickProfile();
      if (!profile) {
        return;
      }
      profile.name = ui.joystickProfileName.value.trim() || profile.name;
      renderJoystickEditor();
    });
  }
  if (ui.joystickControllerType) {
    ui.joystickControllerType.addEventListener("change", () => {
      const profile = getActiveJoystickProfile();
      if (!profile) {
        return;
      }
      profile.controllerType = normalizeJoystickControllerType(ui.joystickControllerType.value);
      profile.bindings = normalizeJoystickBindings(profile.bindings, profile.controllerType);
      renderJoystickEditor();
      renderJoystickLiveInputs();
    });
  }
  if (ui.joystickControllerPort) {
    ui.joystickControllerPort.addEventListener("change", () => {
      const profile = getActiveJoystickProfile();
      if (!profile) {
        return;
      }
      profile.controllerPort = clampJoystickPort(ui.joystickControllerPort.value);
      renderJoystickEditor();
      renderJoystickLiveInputs();
    });
  }
  if (ui.joystickProfileNewButton) {
    ui.joystickProfileNewButton.addEventListener("click", createJoystickProfile);
  }
  if (ui.joystickProfileDuplicateButton) {
    ui.joystickProfileDuplicateButton.addEventListener("click", duplicateJoystickProfile);
  }
  if (ui.joystickProfileDeleteButton) {
    ui.joystickProfileDeleteButton.addEventListener("click", deleteJoystickProfile);
  }
...
const DRIVER_TARGETS = {
  DRIVE_STRAFE: "drive.strafe",
  DRIVE_FORWARD: "drive.forward",
  DRIVE_ROTATE: "drive.rotate",
  ALIGN: "action:alignWithBall",
  SHOOT: "action:shootToggle",
  INTAKE_HOLD: "action:intakeRollersHold",
  INTAKE_DEPLOY: "action:intakeDeployToggle",
  RESET_ODOMETRY: "action:resetOdometry",
};

const JOYSTICK_DEFAULT_TARGETS = [
  {
    id: DRIVER_TARGETS.DRIVE_STRAFE,
    label: "Drive Strafe",
    category: "axis",
    subsystem: "swerve",
    description: "Continuous left/right drivetrain translation request.",
  },
  {
    id: DRIVER_TARGETS.DRIVE_FORWARD,
    label: "Drive Forward/Reverse",
    category: "axis",
    subsystem: "swerve",
    description: "Continuous forward/reverse drivetrain translation request.",
  },
  {
    id: DRIVER_TARGETS.DRIVE_ROTATE,
    label: "Drive Rotate",
    category: "axis",
    subsystem: "swerve",
    description: "Continuous drivetrain rotation request.",
  },
  {
    id: DRIVER_TARGETS.SHOOT,
    label: "Shoot Toggle",
    category: "action",
    subsystem: "superstructure",
    description: "Toggle the shoot enable gate.",
  },
  {
    id: DRIVER_TARGETS.INTAKE_HOLD,
    label: "Intake Rollers Hold",
    category: "action",
    subsystem: "superstructure",
    description: "Hold the intake rollers while active.",
  },
  {
    id: DRIVER_TARGETS.INTAKE_DEPLOY,
    label: "Intake Deploy Toggle",
    category: "action",
    subsystem: "superstructure",
    description: "Toggle intake deploy state.",
  },
  {
    id: DRIVER_TARGETS.RESET_ODOMETRY,
    label: "Reset Odometry",
    category: "action",
    subsystem: "swerve",
    description: "Reset field pose reference.",
  },
  {
    id: DRIVER_TARGETS.ALIGN,
    label: "Align With Fuel",
    category: "action",
    subsystem: "vision",
    description: "Run assisted alignment toward detected game pieces.",
  },
];
...
const CONTROLLER_PRESETS = {
  xbox: {
    ...
    skin: "xbox",
    slots: [
      { id: "l2", kind: "axis", label: "LT", x: 14, y: 6, role: "trigger" },
      { id: "r2", kind: "axis", label: "RT", x: 86, y: 6, role: "trigger" },
      { id: "l1", kind: "button", label: "LB", x: 22, y: 12, role: "shoulder" },
      { id: "r1", kind: "button", label: "RB", x: 78, y: 12, role: "shoulder" },
      { id: "back", kind: "button", label: "View", x: 43, y: 28, role: "center" },
      { id: "menu", kind: "button", label: "Menu", x: 57, y: 28, role: "center" },
      { id: "home", kind: "button", label: "Xbox", x: 50, y: 36, role: "center" },
      { id: "left-x", kind: "axis", label: "Left Stick X", x: 25, y: 53, role: "stick" },
      { id: "left-y", kind: "axis", label: "Left Stick Y", x: 25, y: 66, role: "stick" },
      { id: "right-x", kind: "axis", label: "Right Stick X", x: 72, y: 58, role: "stick" },
      { id: "right-y", kind: "axis", label: "Right Stick Y", x: 72, y: 71, role: "stick" },
      { id: "west", kind: "button", label: "X", x: 70, y: 41, role: "face" },
      { id: "north", kind: "button", label: "Y", x: 77, y: 34, role: "face" },
      { id: "east", kind: "button", label: "B", x: 84, y: 41, role: "face" },
      { id: "south", kind: "button", label: "A", x: 77, y: 48, role: "face" },
    ],
  },
  ps4: {
    id: "ps4",
    ...
    skin: "ps4",
    slots: [
      { id: "l2", kind: "axis", label: "L2", x: 14, y: 6, role: "trigger" },
      { id: "r2", kind: "axis", label: "R2", x: 86, y: 6, role: "trigger" },
      { id: "l1", kind: "button", label: "L1", x: 22, y: 13, role: "shoulder" },
      { id: "r1", kind: "button", label: "R1", x: 78, y: 13, role: "shoulder" },
      { id: "back", kind: "button", label: "Share", x: 39, y: 31, role: "center" },
      { id: "menu", kind: "button", label: "Options", x: 61, y: 31, role: "center" },
      { id: "touchpad", kind: "button", label: "Touchpad", x: 50, y: 24, role: "touchpad" },
      { id: "left-x", kind: "axis", label: "Left Stick X", x: 31, y: 63, role: "stick" },
      { id: "left-y", kind: "axis", label: "Left Stick Y", x: 31, y: 76, role: "stick" },
      { id: "right-x", kind: "axis", label: "Right Stick X", x: 69, y: 63, role: "stick" },
      { id: "right-y", kind: "axis", label: "Right Stick Y", x: 69, y: 76, role: "stick" },
      { id: "west", kind: "button", label: "Square", x: 70, y: 42, role: "face" },
      { id: "north", kind: "button", label: "Triangle", x: 77, y: 35, role: "face" },
      { id: "east", kind: "button", label: "Circle", x: 84, y: 42, role: "face" },
      { id: "south", kind: "button", label: "Cross", x: 77, y: 49, role: "face" },
    ],
  },
  "ps5-pro": {
    id: "ps5-pro",
    ...
    slots: [
      ...
      { id: "l1", kind: "button", label: "L1", x: 22, y: 13, role: "shoulder" },
      { id: "r1", kind: "button", label: "R1", x: 78, y: 13, role: "shoulder" },
      { id: "back", kind: "button", label: "Create", x: 40, y: 31, role: "center" },
      { id: "menu", kind: "button", label: "Options", x: 60, y: 31, role: "center" },
      { id: "touchpad", kind: "button", label: "Touchpad", x: 50, y: 24, role: "touchpad" },
      { id: "left-x", kind: "axis", label: "Left Stick X", x: 31, y: 63, role: "stick" },
      { id: "left-y", kind: "axis", label: "Left Stick Y", x: 31, y: 76, role: "stick" },
      { id: "right-x", kind: "axis", label: "Right Stick X", x: 69, y: 63, role: "stick" },
      ...
      { id: "east", kind: "button", label: "Circle", x: 84, y: 42, role: "face" },
      { id: "south", kind: "button", label: "Cross", x: 77, y: 49, role: "face" },
      { id: "left-paddle", kind: "button", label: "L Paddle", x: 41, y: 92, role: "paddle" },
      { id: "right-paddle", kind: "button", label: "R Paddle", x: 59, y: 92, role: "paddle" },
    ],
  },
};

function normalizeJoystickControllerType(value) {
  const normalized = String(value || "")
    .trim()
    .toLowerCase()
    .replace(/[_\s]+/g, "-");
  if (normalized === "xbox" || normalized === "sim-xbox-universal" || normalized === "simxboxuniversal") {
    return "xbox";
  }
  if (normalized === "ps4" || normalized === "dualshock-4" || normalized === "dualshock4") {
    return "ps4";
  }
  if (
    normalized === "ps5" ||
    ...
    normalized === "dualsense-edge" ||
    normalized === "dualsenseedge" ||
    normalized === "generic-gamepad" ||
    normalized === "genericgamepad"
  ) {
    return "ps5-pro";
  }
  return "ps5-pro";
...
function normalizeJoystickInputKind(value) {
  const normalized = String(value || "")
    .trim()
    .toLowerCase();
  return normalized === "trigger" ? "axis" : normalized || "button";
}

function normalizeJoystickInputId(_kind, value) {
  const normalized = String(value || "")
    .trim()
    .toLowerCase()
    .replace(/[_\s]+/g, "-");
  const aliases = {
    ...
    ps: "home",
    "touch-pad": "touchpad",
    "left-back-button": "left-paddle",
    "right-back-button": "right-paddle",
    leftbackbutton: "left-paddle",
    rightbackbutton: "right-paddle",
  };
  return aliases[normalized] || normalized;
}

function clampJoystickPort(value) {
  const port = Number.parseInt(value, 10);
  if (!Number.isFinite(port)) {
    return 0;
  }
  return Math.max(0, Math.min(5, port));
}

function getControllerPreset(controllerType) {
  return CONTROLLER_PRESETS[normalizeJoystickControllerType(controllerType)] || CONTROLLER_PRESETS["ps5-pro"];
}

function createJoystickBinding(inputKind, inputId, displayLabel, targetId, targetLabel, enabled, notes, targetType) {
  return {
    inputKind,
    inputId,
    displayLabel,
...
function buildDefaultJoystickBindings(controllerType) {
  const preset = getControllerPreset(controllerType);
  const base = [
    createJoystickBinding("axis", "left-x", "Left Stick X", DRIVER_TARGETS.DRIVE_STRAFE, "Drive Strafe", true, "Continuous drivetrain axis.", "axis"),
    createJoystickBinding("axis", "left-y", "Left Stick Y", DRIVER_TARGETS.DRIVE_FORWARD, "Drive Forward/Reverse", true, "Continuous drivetrain axis.", "axis"),
    createJoystickBinding("axis", "right-x", "Right Stick X", DRIVER_TARGETS.DRIVE_ROTATE, "Drive Rotate", true, "Continuous drivetrain axis.", "axis"),
  ];
  if (preset.id === "xbox") {
    return [
      ...base,
      createJoystickBinding("axis", "l2", "LT", DRIVER_TARGETS.ALIGN, "Align With Fuel", true, "Hold to run assisted alignment.", "action"),
      createJoystickBinding("axis", "r2", "RT", DRIVER_TARGETS.SHOOT, "Shoot Toggle", true, "Hold to enable the shoot gate.", "action"),
      createJoystickBinding("button", "r1", "RB", DRIVER_TARGETS.INTAKE_HOLD, "Intake Rollers Hold", true, "Hold to keep intake rollers active.", "action"),
      createJoystickBinding("button", "l1", "LB", DRIVER_TARGETS.INTAKE_DEPLOY, "Intake Deploy Toggle", true, "Toggle intake deploy.", "action"),
      createJoystickBinding("button", "back", "View", DRIVER_TARGETS.RESET_ODOMETRY, "Reset Odometry", false, "Disabled by default to prevent accidental field resets.", "action"),
    ];
  }
  if (preset.id === "ps4") {
    return [
      ...base,
      createJoystickBinding("button", "east", "Circle", DRIVER_TARGETS.ALIGN, "Align With Fuel", true, "Hold to run assisted alignment.", "action"),
      createJoystickBinding("button", "south", "Cross", DRIVER_TARGETS.SHOOT, "Shoot Toggle", true, "Hold to enable the shoot gate.", "action"),
      createJoystickBinding("button", "r1", "R1", DRIVER_TARGETS.INTAKE_HOLD, "Intake Rollers Hold", true, "Hold to keep intake rollers active.", "action"),
      createJoystickBinding("button", "north", "Triangle", DRIVER_TARGETS.INTAKE_DEPLOY, "Intake Deploy Toggle", true, "Toggle intake deploy.", "action"),
      createJoystickBinding("button", "menu", "Options", DRIVER_TARGETS.RESET_ODOMETRY, "Reset Odometry", false, "Disabled by default to prevent accidental field resets.", "action"),
    ];
  }
  return [
    ...base,
    createJoystickBinding("axis", "l2", "L2", DRIVER_TARGETS.ALIGN, "Align With Fuel", true, "Hold to run assisted alignment.", "action"),
    createJoystickBinding("axis", "r2", "R2", DRIVER_TARGETS.SHOOT, "Shoot Toggle", true, "Hold to enable the shoot gate.", "action"),
    createJoystickBinding("button", "r1", "R1", DRIVER_TARGETS.INTAKE_HOLD, "Intake Rollers Hold", true, "Hold to keep intake rollers active.", "action"),
    createJoystickBinding("button", "l1", "L1", DRIVER_TARGETS.INTAKE_DEPLOY, "Intake Deploy Toggle", true, "Toggle intake deploy.", "action"),
    createJoystickBinding("button", "menu", "Options", DRIVER_TARGETS.RESET_ODOMETRY, "Reset Odometry", false, "Disabled by default to prevent accidental field resets.", "action"),
  ];
}

function createDefaultJoystickProfile(controllerType, profileId, name) {
  const preset = getControllerPreset(controllerType);
  return {
    id: profileId,
    name: name || `${preset.label} Driver`,
    controllerType: preset.id,
    controllerPort: 0,
    deviceName: "Driver Controller",
    enabled: true,
    bindings: buildDefaultJoystickBindings(preset.id),
    warnings: [],
  };
}

function ensureJoystickTargets(targets) {
  const merged = new Map();
  JOYSTICK_DEFAULT_TARGETS.forEach((target) => merged.set(target.id, { ...target }));
...
function normalizeJoystickBindings(bindings, controllerType) {
  const preset = getControllerPreset(controllerType);
  return (Array.isArray(bindings) ? bindings : [])
    .filter(Boolean)
    .map((binding) => {
      const inputKind = normalizeJoystickInputKind(binding.inputKind);
      const inputId = normalizeJoystickInputId(inputKind, binding.inputId);
      const slot = preset.slots.find((entry) => entry.id === inputId && entry.kind === inputKind);
      return {
        ...binding,
        inputKind,
        inputId,
        displayLabel: slot?.label || binding.displayLabel || `${inputKind}:${inputId}`,
        enabled: binding.enabled !== false,
      };
    });
}

function normalizeJoystickProfile(profile, index) {
  const controllerType = normalizeJoystickControllerType(profile?.controllerType);
  const normalizedBindings = normalizeJoystickBindings(profile?.bindings, controllerType);
  return {
    id: profile?.id || `driver-profile-${index + 1}`,
    name: profile?.name || `${getControllerPreset(controllerType).label} Driver`,
    controllerType,
    controllerPort: clampJoystickPort(profile?.controllerPort),
    deviceName: profile?.deviceName || "Driver Controller",
    enabled: profile?.enabled !== false,
    bindings: normalizedBindings.length ? normalizedBindings : buildDefaultJoystickBindings(controllerType),
    warnings: Array.isArray(profile?.warnings) ? profile.warnings.filter(Boolean) : [],
  };
}

function normalizeJoystickMappingsDocument(document) {
  const normalized = cloneJson(document) || {};
  normalized.targets = ensureJoystickTargets(normalized.targets);
  normalized.profiles = (Array.isArray(normalized.profiles) ? normalized.profiles : []).map((profile, index) =>
    normalizeJoystickProfile(profile, index)
  );
  if (!normalized.profiles.length) {
    normalized.profiles = [
      createDefaultJoystickProfile("xbox", "xbox-driver", "Xbox Driver"),
      createDefaultJoystickProfile("ps4", "ps4-driver", "PS4 Driver"),
      createDefaultJoystickProfile("ps5-pro", "ps5-pro-driver", "PS5 Pro Driver"),
    ];
  }
  normalized.activeProfileId =
    normalized.profiles.find((profile) => profile.id === normalized.activeProfileId)?.id ||
...
function setActiveJoystickProfile(profileId) {
  if (!state.joystickMappings) {
    return;
  }
  const nextProfile = state.joystickMappings.profiles?.find((entry) => entry.id === profileId);
  if (!nextProfile) {
    return;
  }
  state.selectedJoystickProfileId = nextProfile.id;
  state.joystickMappings.activeProfileId = nextProfile.id;
}

function findJoystickBinding(profile, inputKind, inputId) {
  const kind = normalizeJoystickInputKind(inputKind);
  const normalizedId = normalizeJoystickInputId(kind, inputId);
  return (Array.isArray(profile?.bindings) ? profile.bindings : []).find(
    (binding) =>
      normalizeJoystickInputKind(binding.inputKind) === kind &&
      normalizeJoystickInputId(binding.inputKind, binding.inputId) === normalizedId
  );
}
...
function computeJoystickConflicts(profile) {
  ...
  profile.bindings.forEach((binding) => {
    ...
    const key = `${normalizeJoystickInputKind(binding.inputKind)}:${normalizeJoystickInputId(binding.inputKind, binding.inputId)}`;
...
function syncActiveJoystickProfileWarnings() {
  ...
  profile.warnings = conflicts.length
    ? conflicts
    : [`No conflicting enabled bindings detected for ${profile.name}.`];
  return conflicts;
}

function nextJoystickProfileId(baseName) {
  const slug = String(baseName || "driver-profile")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "") || "driver-profile";
  const existing = new Set((state.joystickMappings?.profiles || []).map((entry) => entry.id));
  let candidate = slug;
  let index = 2;
...
function createJoystickProfile() {
  if (!state.joystickMappings) {
    return;
  }
  const seedType = getActiveJoystickProfile()?.controllerType || "ps5-pro";
  const id = nextJoystickProfileId(`${seedType}-driver`);
  const profile = createDefaultJoystickProfile(seedType, id, `${getControllerPreset(seedType).label} Driver Copy`);
  state.joystickMappings.profiles.push(profile);
  setActiveJoystickProfile(profile.id);
  renderJoystickEditor();
  renderJoystickLiveInputs();
}

function duplicateJoystickProfile() {
  const profile = getActiveJoystickProfile();
  if (!profile || !state.joystickMappings) {
    return;
  }
  const copy = cloneJson(profile);
  copy.id = nextJoystickProfileId(profile.id);
  copy.name = `${profile.name} Copy`;
  state.joystickMappings.profiles.push(copy);
  setActiveJoystickProfile(copy.id);
  renderJoystickEditor();
  renderJoystickLiveInputs();
}

function deleteJoystickProfile() {
  if (!state.joystickMappings) {
    return;
  }
  const profiles = state.joystickMappings.profiles || [];
  if (profiles.length <= 1) {
    showToast("Keep at least one controller profile");
    return;
  }
  const activeId = getActiveJoystickProfile()?.id;
  state.joystickMappings.profiles = profiles.filter((profile) => profile.id !== activeId);
  setActiveJoystickProfile(state.joystickMappings.profiles[0]?.id || "");
  renderJoystickEditor();
  renderJoystickLiveInputs();
}

function assignJoystickTargetToSlot(profile, slot, target) {
  if (!profile || !slot || !target) {
    return;
  }
  const existing = findJoystickBinding(profile, slot.kind, slot.id);
  const next = existing || {
    inputKind: slot.kind,
    inputId: slot.id,
  ...
  next.inputKind = slot.kind;
  next.inputId = slot.id;
  next.displayLabel = slot.label;
  next.targetId = target.id;
  next.targetLabel = target.label;
  next.targetType = target.category;
  next.enabled = true;
  next.notes = target.description || next.notes || `Assigned from the ${profile.name} controller preview.`;
  if (!existing) {
    profile.bindings.push(next);
  }
}

function clearJoystickSlot(profile, slot) {
  if (!profile || !Array.isArray(profile.bindings)) {
    return;
  }
  profile.bindings = profile.bindings.filter(
    (binding) =>
      !(
        normalizeJoystickInputKind(binding.inputKind) === normalizeJoystickInputKind(slot.kind) &&
        normalizeJoystickInputId(binding.inputKind, binding.inputId) === normalizeJoystickInputId(slot.kind, slot.id)
      )
  );
}

function renderJoystickTargetLibrary(targets) {
  if (!ui.joystickTargetLibrary) {
    return;
  }
  ui.joystickTargetLibrary.innerHTML = "";
  const sortedTargets = [...targets].sort((a, b) => {
    const categoryCompare = String(a.category || "").localeCompare(String(b.category || ""));
    return categoryCompare || String(a.label || a.id || "").localeCompare(String(b.label || b.id || ""));
  });
  sortedTargets.forEach((target) => {
    const card = document.createElement("div");
    card.className = "joystick-target-card";
    card.draggable = true;
    card.innerHTML = `<div class="joystick-target-card__type">${target.category || "command"}</div><div class="joystick-target-card__label">${target.label || target.id}</div><div class="joystick-target-card__detail">${target.subsystem || "general"} • ${target.id}</div>`;
    card.addEventListener("dragstart", (event) => {
      state.joystickDraggedTargetId = target.id;
      event.dataTransfer?.setData("text/plain", target.id);
      event.dataTransfer?.setData("application/x-operatorboard-target", target.id);
    });
    card.addEventListener("dragend", () => {
      state.joystickDraggedTargetId = "";
    });
    ui.joystickTargetLibrary.append(card);
  });
}

function renderJoystickControllerPreview(profile, preset, targetsById) {
  if (!ui.joystickControllerPreview) {
    return;
  }
  ui.joystickControllerPreview.innerHTML = "";
  const canvas = document.createElement("div");
  canvas.className = `controller-preview controller-preview--${preset.skin}`;
  preset.slots.forEach((slot) => {
    const binding = findJoystickBinding(profile, slot.kind, slot.id);
    const target = binding?.targetId ? targetsById.get(binding.targetId) : null;
    const slotCard = document.createElement("div");
    slotCard.className = `controller-slot controller-slot--${slot.role || "button"}${target ? " is-bound" : ""}${binding && binding.enabled === false ? " is-disabled" : ""}`;
    slotCard.style.left = `${slot.x}%`;
    slotCard.style.top = `${slot.y}%`;
    slotCard.innerHTML = `<div class="controller-slot__name">${slot.label}</div><div class="controller-slot__assignment">${target?.label || "Drop command"}</div>`;
    slotCard.addEventListener("dragover", (event) => {
      event.preventDefault();
      slotCard.classList.add("is-drop-target");
    });
    slotCard.addEventListener("dragleave", () => slotCard.classList.remove("is-drop-target"));
    slotCard.addEventListener("drop", (event) => {
      event.preventDefault();
      slotCard.classList.remove("is-drop-target");
      const targetId =
        event.dataTransfer?.getData("application/x-operatorboard-target") ||
        event.dataTransfer?.getData("text/plain");
      const nextTarget = targetsById.get(targetId);
      if (!nextTarget) {
        return;
      }
      assignJoystickTargetToSlot(profile, slot, nextTarget);
      renderJoystickEditor();
    });
    if (target) {
      const clearButton = document.createElement("button");
      clearButton.type = "button";
      clearButton.className = "controller-slot__clear";
      clearButton.textContent = "Clear";
      clearButton.addEventListener("click", () => {
        clearJoystickSlot(profile, slot);
        renderJoystickEditor();
      });
      slotCard.append(clearButton);
    }
    canvas.append(slotCard);
  });
  ui.joystickControllerPreview.append(canvas);
}

function renderJoystickBindingTable(profile, preset, targetsById) {
  ...
  preset.slots.forEach((slot) => {
    ...
    slotCell.className = "mapping-row__cell";
    slotCell.textContent = slot.label;

    const targetCell = document.createElement("div");
    targetCell.className = "mapping-row__cell mapping-row__cell--wide";
    const targetValue = document.createElement("div");
    targetValue.className = "mapping-row__target";
    targetValue.textContent = target?.label || "Unassigned";
    const targetDetail = document.createElement("div");
    targetDetail.className = "mapping-row__detail";
    targetDetail.textContent = target ? `${target.category} • ${target.id}` : "Drop a command onto this slot in the preview.";
    targetCell.append(targetValue, targetDetail);

    if (binding) {
      const clearButton = document.createElement("button");
      clearButton.type = "button";
      clearButton.className = "mapping-row__clear";
      clearButton.textContent = "Clear";
      clearButton.addEventListener("click", () => {
        clearJoystickSlot(profile, slot);
        renderJoystickEditor();
      });
      targetCell.append(clearButton);
    }
    ...
    enabledCell.append(enabled, document.createTextNode(binding ? (binding.enabled ? "Enabled" : "Disabled") : "Empty"));

    row.append(slotCell, targetCell, enabledCell);
...
function renderJoystickEditor() {
  ...
  const profile = getActiveJoystickProfile();
  const preset = getControllerPreset(profile?.controllerType);
  ...
  if (ui.joystickProfileName) {
    ui.joystickProfileName.value = profile?.name || "";
  }
  if (ui.joystickControllerType) {
    ui.joystickControllerType.innerHTML = "";
    Object.values(CONTROLLER_PRESETS).forEach((controllerPreset) => {
      const option = document.createElement("option");
      option.value = controllerPreset.id;
      option.textContent = controllerPreset.label;
      option.selected = controllerPreset.id === preset.id;
      ui.joystickControllerType.append(option);
    });
  }
  if (ui.joystickControllerPort) {
    ui.joystickControllerPort.innerHTML = "";
    for (let port = 0; port <= 5; port += 1) {
      const option = document.createElement("option");
      option.value = String(port);
      option.textContent = `Port ${port}`;
      option.selected = port === clampJoystickPort(profile?.controllerPort);
      ui.joystickControllerPort.append(option);
    }
  }

  setText(
    ui.joystickProfileStatus,
    doc ? `Loaded ${doc.profiles?.length || 0} profile(s) • Active ${profile?.name || "None"}` : "Unavailable"
  );
  const conflicts = syncActiveJoystickProfileWarnings();
  setText(
    ui.joystickConflictStatus,
    !profile ? "No profile selected" : conflicts.length ? `${conflicts.length} conflict(s)` : "No conflicts"
  );
  if (ui.joystickWarningList) {
    ...
    const warnings = Array.isArray(profile?.warnings) && profile.warnings.length
      ? profile.warnings
      : ["Drag a command onto the preview to assign a controller slot."];
  ...
  renderJoystickTargetLibrary(targets);
  if (!profile) {
    if (ui.joystickControllerPreview) {
      ui.joystickControllerPreview.innerHTML = '<div class="checks-list__empty">No active controller profile.</div>';
    }
    if (ui.joystickBindingTable) {
      ui.joystickBindingTable.innerHTML = '<div class="checks-list__empty">No bindings available.</div>';
    }
    return;
  }

  renderJoystickControllerPreview(profile, preset, targetsById);
  renderJoystickBindingTable(profile, preset, targetsById);
}

function resolvePreferredBrowserGamepad() {
  const profile = getActiveJoystickProfile();
  const selectedPort = clampJoystickPort(profile?.controllerPort);
  const gamepads = Array.from(navigator.getGamepads?.() || []);
  return {
    selectedPort,
    gamepad:
      gamepads.find((entry) => entry && entry.index === selectedPort) ||
      gamepads[selectedPort] ||
      gamepads.find(Boolean) ||
      null,
  };
}

function renderJoystickLiveInputs() {
  const { selectedPort, gamepad } = resolvePreferredBrowserGamepad();
  const activeProfile = getActiveJoystickProfile();
  setText(ui.joystickControllerStatus, gamepad ? `Controller online • Port ${selectedPort}` : `Port ${selectedPort} offline`);
  setText(ui.joystickDeviceId, gamepad?.id || activeProfile?.deviceName || "--");
...
async function saveJoystickMappings(successMessage = "Joystick mappings saved") {
  if (!state.joystickMappings) {
  ...
  state.joystickMappings = normalizeJoystickMappingsDocument(saved);
  state.joystickMappingsBaseline = cloneJson(state.joystickMappings);
  setActiveJoystickProfile(state.joystickMappings.activeProfileId);
  renderJoystickEditor();
  renderJoystickLiveInputs();
...
function resetJoystickMappings() {
  ...
  state.joystickMappings = normalizeJoystickMappingsDocument(cloneJson(state.joystickMappingsBaseline));
  setActiveJoystickProfile(state.joystickMappings.activeProfileId);
  renderJoystickEditor();
  renderJoystickLiveInputs();
