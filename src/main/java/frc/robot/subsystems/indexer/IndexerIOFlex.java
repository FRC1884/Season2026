...
import frc.robot.generic.rollers.GenericRollerSystemIO.GenericRollerSystemIOInputs;

public class IndexerIOFlex implements IndexerIO {
  ...
  public IndexerIOFlex() {
    ...
    leader = new SparkFlex(IndexerConstants.LEADER_ID, MotorType.kBrushless);
    leader.configure(config, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);

    if (IndexerConstants.FOLLOWER_ID >= 0) {
      follower = new SparkFlex(IndexerConstants.FOLLOWER_ID, MotorType.kBrushless);
      follower.configure(
          new SparkFlexConfig().apply(config).follow(leader, IndexerConstants.FOLLOWER_INVERTED),
